...
if [ -n "${AGE_RECIPIENT:-}" ] && command -v age >/dev/null 2>&1; then
  age -r "$AGE_RECIPIENT" -o "$BACKUP_FILE.age" "$BACKUP_FILE" && echo "[encrypt] $BACKUP_FILE.age"
fi

# Optional: S3/R2 upload if configured (requires aws cli + env)
if [ -n "${S3_BUCKET:-}" ] && command -v aws >/dev/null 2>&1; then
  # shellcheck disable=SC2086
  aws s3 cp "$BACKUP_FILE" "s3://${S3_BUCKET}/daily/$(basename "$BACKUP_FILE")" ${AWS_S3_EXTRA_ARGS:-} && echo "[upload] s3://${S3_BUCKET}/daily/$(basename "$BACKUP_FILE")"
fi

echo "[backup] done $BACKUP_FILE"
ls -lh "$BACKUP_FILE" "$BACKUP_FILE.sha256"

# Retention: keep 7 daily local (simple)
RETENTION_DAYS="${RETENTION_DAYS:-7}"
if [ "$RETENTION_DAYS" -gt 0 ] 2>/dev/null; then
  find "$BACKUP_DIR" -name 'yotara-*.db' -mtime +"$RETENTION_DAYS" -delete -print 2>/dev/null | head -5 || true
